...
/**
  * This provides an event sourced behavior. It has a state, [[MurakumoState]], which
  ...
  */
object MurakumoBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[MurakumoCommand]): Behavior[MurakumoCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, MurakumoEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[MurakumoCommand, MurakumoEvent, MurakumoState](
        persistenceId = persistenceId,
        emptyState = MurakumoState.initial,
...
/**
  ...
  */
case class MurakumoState(message: String, timestamp: String) {
  def applyCommand(cmd: MurakumoCommand): ReplyEffect[MurakumoEvent, MurakumoState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: MurakumoEvent): MurakumoState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[MurakumoEvent, MurakumoState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[MurakumoEvent, MurakumoState] =
    Effect
...
object MurakumoState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: MurakumoState = MurakumoState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[MurakumoCommand]("MurakumoAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[MurakumoState] = Json.format
}

/**
  * This interface defines all the events that the MurakumoAggregate supports.
  */
sealed trait MurakumoEvent extends AggregateEvent[MurakumoEvent] {
  def aggregateTag: AggregateEventTag[MurakumoEvent] = MurakumoEvent.Tag
}

object MurakumoEvent {
  val Tag: AggregateEventTag[MurakumoEvent] = AggregateEventTag[MurakumoEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MurakumoEvent
...
/**
  ...
  */
trait MurakumoCommandSerializable

/**
  * This interface defines all the commands that the MurakumoAggregate supports.
  */
sealed trait MurakumoCommand
    extends MurakumoCommandSerializable
...
/**
  ...
  * application loader.
  */
object MurakumoSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MurakumoState],
